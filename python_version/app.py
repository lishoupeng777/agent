...
## Few-shot 标注示例（严格参考评分逻辑和松紧程度）

示例1 - 良好治理：
治理前: 尊敬的客户您好！您的订单[NO.20240315-88921]已发货。预计送达时间：2024年3月18日 下午。如有问题 请致电 400-888-9999 。
治理后: 尊敬的客户您好！您的订单[NO.20240315-88921]已发货。预计送达时间：2024年3月18日下午。如有问题请致电400-888-9999。
正确评分: {"semantic_consistency_score":1.0, "readability_structure_score":1.0, "flaws":[], "overall_explanation":"仅修正多余空格，语义和结构完整。"}

示例2 - 严重过度清洗：
治理前: 本次产品升级主要包括三个方面：第一，界面全新改版，采用卡片式布局；第二，新增智能推荐功能；第三，性能大幅优化，页面加载速度提升50%以上。
治理后: 本次产品进行了升级。
正确评分: {"semantic_consistency_score":0.15, "readability_structure_score":1.0, "flaws":[{"anchor":"[Before 1] -> [After 1]","type":"过度清洗","severity":"High","detail":"三条具体升级要点全部删除，核心信息完全丢失。按累计扣分规则，总分不得超过0.2。"}], "overall_explanation":"治理后文本将三条具体要点全部删除，仅剩空洞概括，语义保真度极低。"}
...
def _make_cache_key(before_text: str, after_text: str) -> str:
    """基于输入文本生成 SHA256 缓存键"""
    payload = before_text + "|||" + after_text
...
def run_evaluation(
    ...
    """
    # ---- 0. 缓存检查 ----
    cache_key = _make_cache_key(before_text, after_text)
    cache_store = st.session_state.setdefault("eval_cache", {})
    if cache_key in cache_store:
        cached = cache_store[cache_key].copy()
