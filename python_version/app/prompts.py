...
- 如果仅删除举例、具体措施、非核心修饰语，但核心信息完整保留，semantic 不宜低于 0.6
...
**【约束词删除检测 — 必须重点关注】**
以下词汇属于政策/法规/合同中的约束条件，一旦被删除或泛化，factual 和 semantic 必须降低：
...
如果治理后文本删除了上述约束词，即使"大意保留"，也属于严重事实变更，factual ≤ 0.4，semantic ≤ 0.5。
...
如果治理后文本删除或泛化了定义条款中的限定词（如"书面同意"→"同意"，"明确标注为保密"→"相关信息"），属于定义条款弱化，factual ≤ 0.4，semantic ≤ 0.5。
...
- "国家电网有限公司"→"国网公司" → factual ≤ 0.3, 标为 critical
- "南方电网有限责任公司"→"南网公司" → factual ≤ 0.3, 标为 critical
- "中华人民共和国教育部"→"教育部" → 仅限口语化场景，政务文本中仍属不规范
技术参数必须精确，不得四舍五入或近似：
- ±1100千伏→±1000千伏 → factual ≤ 0.2, 标为 critical
...
- "书面同意"→"同意" → 删除了方式限定，factual ≤ 0.3, 标为 critical
- "明确标注为保密"→"相关信息" → 删除了认定标准，factual ≤ 0.3, 标为 critical
- "签署前后"→ 删除 → 删除了时间范围限定，factual ≤ 0.4, 标为 major
- "以书面或口头形式获悉"→ 删除 → 删除了获悉方式限定，factual ≤ 0.4, 标为 major
